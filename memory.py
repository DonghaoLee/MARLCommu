...
class SequentialMemory(Memory):
    ...
    def sample_and_split(self, batch_size, batch_idxs=None):
        ...
        for i in batch_idxs:
            state0_batch.append(self.observations[i])
            state1_batch.append(self.observations[i+1])
            reward_batch.append(self.rewards[i])
            action_batch.append(self.actions[i])
            terminal1_batch.append(self.terminals[i])

        # Prepare and validate parameters.
        state0_batch = torch.stack(state0_batch).reshape(batch_size,-1).float()
        state1_batch = torch.stack(state1_batch).reshape(batch_size,-1).float()
